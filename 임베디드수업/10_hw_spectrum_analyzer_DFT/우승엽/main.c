...
COMPLEX temp_array[256];

void iniit_com_array(COMPLEX array[], int size)
{
...
{
	COMPLEX c;
	c.reN = (a.reN * b.reN) - (a.imN * b.imN);
	c.imN = (b.imN * a.reN) - (b.reN * a.imN);
...
COMPLEX com_sub(COMPLEX a, COMPLEX b)
{
	COMPLEX c;
	c.reN = (a.reN - b.reN);
	c.imN = (a.imN - b.imN);
...
float com_mag(COMPLEX a)
{
	float c;
	c= sqrt(a.reN*a.reN + a.imN*a.imN);
	return c;
}

int bit_reverse_cal(int n, int bit_width)
{
	int sum = 0;
	int temp;
...
void bit_reverse(float array[], int size, int bit_width)
{
	int index = 0;

	for(int i = 0; i < size; i++)
	{
		index = bit_reverse_cal(i, bit_width);
		temp_array[index].reN = array[i];
	}
}
...
void ADC1_IRQHandler(){
	if( ADC1_SR & 1<<1 ){
		if(sample_count<256) // data gathering
		{
			sample_input_array[sample_count] = ADC1_DR & 0xFF;
			sample_count++;
		}
		else // calculate and send, clear.
		{	
			bit_reverse(sample_input_array, 256, 8);
			...
			for(int i = 2; i <= 256; i<<=1)
			{
				for(int j = 0; j < 256; j += i)
				{
					for(int k =0; k< i/2; k++)
					{
						...
						Wk.imN = sin(-2*PI*k/i);
						even = temp_array[j+k];
						odd = temp_array[j+k+i/2];
						temp_array[j+k] = com_add(even, com_multi(odd, Wk));
						temp_array[j+k+i/2] = com_sub(even, com_multi(odd, Wk)); 
					}
				}
			}

			for (int i = 0; i<256; i++){
				len = sprintf(buf, "%.1lf\n",com_mag(temp_array[i]));
