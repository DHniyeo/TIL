...
void sendStr(char buffer[], int max);
...
float V;
const float MinV = 0.4;
const float MaxV = 2.8;
...
/* Led state about Distance */
void Led_state_about_Distance(){
	GPIOD_ODR &= ~(1 << 12);	
	GPIOD_ODR &= ~(1 << 13);
	GPIOD_ODR &= ~(1 << 14);
	GPIOD_ODR &= ~(1 << 15);

	if(result_Distance>120)
		GPIOD_ODR |= 1 << 12;
	if(result_Distance> 80)
		GPIOD_ODR |= 1 << 13;
	if(result_Distance> 40)
		GPIOD_ODR |= 1 << 14;
...
void ADC1_IRQHandler(){
	if( (ADC1_SR & 1<<1) && (button_on_off==1) ){
		if(sample_count<Sample_Size) // data gathering (data get and calculate distance.)
		{
			...
			result_Distance += Distance; // sum of Distance
		
			sample_count++;
		}
		else
		{
			result_Distance /= Sample_Size;
			len = sprintf(buf, "%.1lfcm\n", result_Distance);
			sendStr(buf, len);
		
			Led_state_about_Distance(); // LED state change
